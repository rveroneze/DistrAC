...
The `.sh` files contain ready-to-run examples of the federated experiments executed with MLP and LR in the paper's evaluation. These scripts used in the experiments were generated automatically by `create_run_flwr_LR.py` and `create_run_flwr_MLP.py`.

## Generating the Shell Scripts

The two generator scripts read the output of the **centralized training** to set the hyper-parameters for the distributed simulation. This design reflects the experimental setup of the paper: the distributed (federated) simulation leverages knowledge from the centralized training to configure hyper-parameters such as learning rate and number of epochs.
