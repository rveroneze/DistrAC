...
FOUTPUT = '/Documents/AIDE_rinclose_niid_qs/'
...
print('Dataset:', id_uci, 'n_sites:', nsites, 'seed:', seed, 'alpha dirichlet:', alpha_dirichlet)
...
fpath_s = fpath + f'nsites{nsites}/alpha{alpha_dirichlet}/'
frinclose = f'{fpath}RInClose_mminsup'


def get_minsups(id_uci, counts, theta, labels=None):
    ...
    if id_uci not in ids_uci_special:
        ...
        if id_uci == 22:
            ...
            rminsupsn = np.zeros(len(counts))
            for i, label in enumerate(labels):
                rminsupsn[i] = max(rminsups[i], id_69[label])
        elif id_uci == 26:
...
data_name, X_train, X_test, y_train, y_test, _ = my_utils.get_data_uci(id_uci, encoder='ordinal', seed=seed, X_to_numpy=False, stratify=True)
...
rminsups, minsups, rminsupsn, minsupsn = get_minsups(id_uci, n_classes, theta, classes)
...
if nsites > 1:
    clients_data = my_utils.get_partitions_quantity_skew(X_train, y_train, nsites, seed, alpha=alpha_dirichlet)
    
    for ns in range(nsites):
        print(F'SITE {ns} of {nsites-1}')
                
        X_train_local, y_train_local = clients_data[ns]
        v_l, c_l = np.unique(y_train_local, return_counts=True)
        print('Training Class Labels:', v_l, ' Counts:', c_l)
        if X_train_local.shape[0] > 1:
            rminsups, minsups, rminsupsn, minsupsn = get_minsups(id_uci, c_l, theta, v_l)
            print('rminsups:', rminsups, ' minsups:', minsups)
            print('rminsupsn:', rminsupsn, ' minsupsn:', minsupsn, '\n')
